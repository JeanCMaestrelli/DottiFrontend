<template>
    ...
    <div class="container">
        <div id="conteudo" class="Eventos z-depth-1">
            <h5 style="font-weight: bold;">CADASTRO DE CONTAS GERENCIAIS</h5>
            ...
            <div class="painel z-depth-1">
                ...
                <form v-on:submit.prevent="onSubmit">
                    <div class="row">
                        <div class="col s12">
                            <div class="input-field col l2 m2 s12">
                                <input disabled v-model="CODGERENCIAL" id="txt_Codigo" name="txt_Codigo" type="text">
                    ...
                    <br>
                    <div class="row ">
                        <button id="SalvarEvento" @click="salvargerencial($event)" class="waves-effect waves-light btn right btnsEventos">Cadastrar</button>
                        <button id="EditarEvento" @click="editargerencial($event)" class="waves-effect waves-light btn right btnsEventos">Editar</button>
                        <button id="ExcluirEvento" @click="excluirgerencial($event)" class="waves-effect waves-light btn right btnsEventos">Excluir</button>
                    </div>
                </form>
                <br>
            </div>
            ...
            <div class="row">
                <div class="col s12 z-depth-1" id="tableContainer" style="min-height: 400px;">
                    <table  class="centered striped" id="tabDados">
                        ...
                        <tbody>
                            <tr :items="Rows" v-for="gerencial in lstGerencial" :key="gerencial.codgerencial">
                                <td>
                                    <label>
                                    <input type="checkbox" :id="gerencial.codgerencial" v-model="selectedRows" :value="gerencial"/>
                                    <span></span>
                                    </label>
                                </td>
  ...
  export default {
    ...
    computed:{
        Rows() {
            var rows = 0;
            return this.lstGerencial.find(() => {
                rows += 1;
                if(this.lstGerencial.length == rows)
                {
                    setTimeout(  () => {
                        M.updateTextFields();
                        api.loadingOff();
    ...
    methods:
    {
        async salvargerencial(e)
        {
            if(this.CGERENCIAL === "")
            {
                toast.error("Informe o Código Gerencial");
                return false;
            }
            if(this.DESCRICAO === "")
            {
            ...
            if(this.flag)
            {
                var ret1 = await api.verificarAcesso("CGERENCIAL","SALVAR","O seu perfil não possui permissão para salvar dados !!!");
                if(!ret1)
                {
                    return;
                }
                await api.post("cadCGerencial", data).then(r=>{
                if(r.status == 401)
                {
                    api.loadingOff();
                ...
                else
                {
                    ...
                    toast("Código Gerencial Cadastrado com Sucesso !!!");
                }})
                e.preventDefault();
                api.loadingOff();
                return true;
            ...
            {
                api.loadingOn();
                await api.post("updateCGerencial", data).then(r=>
                {
                    if(r.status == 401)
                    {
                        api.loadingOff();
                    ...
                    else
                    {
                        this.LimparCampos();
                        this.getAllgerencial();
                        toast("Código Gerencial Atualizado com Sucesso !!!");
                    }
                });
                e.preventDefault();
                this.flag = true;
        ...
        async editargerencial(e)
        {
            ...
            else if(this.selectedRows.length > 1)
            {
                ...
                toast("Marque somente um código gerencial para editar !!!")
                return;
            }
            else if(this.selectedRows.length == 0)
            {
                toast("Marque um código gerencial para editar !!!")
                return;
            }
            ...
            {
                api.loadingOn();
                var ret = await api.verificarAcesso("CGERENCIAL","EDITAR","O seu perfil não possui permissão para editar dados !!!");
                if(!ret)
                {
                    return;
                }
                document.getElementById("txt_Codigo").value = this.selectedRows[0].codgerencial;
                document.getElementById("txt_Descricao").value = this.selectedRows[0].descricao;
                document.getElementById("txt_gerencial").value = this.selectedRows[0].cgerencial;
        ...
        async excluirgerencial(e)
        {
            e.preventDefault();
            if(this.selectedRows.length > 1)
            {
                toast("Marque somente um código gerencial para excluir !!!")
                return;
            }
            else if(this.selectedRows.length == 0)
            {
                toast("Marque um código gerencial para excluir !!!")
                return;
            }
            ...
            if(this.flagex)
            {
                var ret = await api.verificarAcesso("CGERENCIAL","EXCLUIR","O seu perfil não possui permissão para excluir dados !!!");
                if(!ret)
                {
                    return;
                }
            ...
            else
            {
                api.loadingOn();
                let data = {
                    codgerencial: this.selectedRows[0].codgerencial
                }
                api.delete("deleteCGerencial", data).then(r=>{
                    this.LimparCampos();
                    if(r.status == 401)
                    {
                        api.loadingOff();
                        ...
                        api.loadingOff();
                        toast("Código Gerencial Excluido com Sucesso !!!");
                        this.getAllgerencial();
                        this.LimparCampos();
                    }})
                this.flagex = true;
        ...
        Datas(valor,variavel,gerencial=1)
        {
            if(gerencial == 1)//aplicar mascara
            {
                this[variavel] = api.aplicarMascaraData(valor);
            }
            else if(gerencial == 2)//consistir data
            {
                if(valor) 
                {
